...
from fundus_stiching_toolkit.registration.helper import choose_reference_image
from fundus_stiching_toolkit.segment import segment_od_mask, segment_vessels
from fundus_stiching_toolkit.utils.config import Descriptors, ReferenceChoice
from fundus_stiching_toolkit.utils.visu import imshow
...
class FundusRegistration:
    ...
    def extract_keypoints_from_vessels(self, neighborhood_size=3):
        ...
        vessels_skeletons = [
            skeletonize(mask).astype(np.uint8) * (1 - odmask.astype(np.uint8))
            for mask, odmask in zip(self.masks_vessels, self.masks_od)
        ]
